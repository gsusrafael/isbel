...
class ProcessFile(pyinotify.ProcessEvent):
    def process_IN_CREATE(self, event):
        ...
        try:
            queueLock.acquire()
            workQueue.put(os.path.join(event.path, event.name))
            print workQueue.qsize(), os.path.join(event.path, event.name)
        finally:
            queueLock.release()
...
class FileMonitor(threading.Thread):
    ...
    def run(self):
        while workQueue.empty(): # En una cola sin elementos
    ...
    while not exitFlag:
        ...
        if not workQueue.empty():
            ...
            if run_command:
                ...
                ssh.exec_command('runcbl MFR0167')
                ssh.close()
...
wm.add_watch('/tmp/mirror/', pyinotify.IN_CREATE, rec=True) #@UndefinedVariable
